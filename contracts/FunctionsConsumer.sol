...
contract FunctionsConsumer is FunctionsClient, ConfirmedOwner {
  ...
  // Reference to encrypted secrets passed into the source code which are accessible via the global object `secrets`
  ...
  /**
   * @notice Sets the secrets for the Functions request
   * @param encryptedSecretsReference Reference to the DONHosted encrypted secrets generated by the SecretsManager.uploadEncryptedSecretsToDON() method
   * (See https://github.com/smartcontractkit/functions-toolkit?tab=readme-ov-file#don-hosted-secrets)
   */
  function setEncryptedSecretsReference(bytes calldata encryptedSecretsReference) external onlyOwner {
    s_encryptedSecretsReference = encryptedSecretsReference;
  }
  ...
  function sendRequest(string[] calldata args) external onlyOwner {
    ...
    if (s_encryptedSecretsReference.length > 0) {
      // This example only shows DONHosted secrets, but Remote secrets are also supported
      // (See https://docs.chain.link/chainlink-functions/tutorials/api-use-secrets-gist)
